...
def main(load_model_path: str):
    ...
    data_module.prepare_data(loss_image=False)
    ...
    # FIXME: a hack
    print(f"Running {run_cfg.data}\n\tforbidden folder {run_cfg.data.data_path+'/forbidden_prototypes'}", flush=True)
    forbidden_folders = os.listdir(run_cfg.data.data_path+'/forbidden_prototypes')
    if len(forbidden_folders) > 0: 
        print('Switch True', flush=True)
        switch = True
    else:
        print('Switch False', flush=True)
        switch = False
    data_module = DATASETS[run_cfg.data.name](run_cfg, None, global_analysis=True)
    data_module.prepare_data(loss_image = switch)
    dataset = data_module.get_dataset()

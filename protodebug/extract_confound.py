...
def patches_extraction(path: str,
    ...
    user_exp_path = os.path.join(os.path.dirname(path), 'user_experiment')
    ...
    model_suffix = path.split('/')[-1].split('.pth.tar')[0]
    img_class_idxs = np.load(os.path.join(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', "full_class_id.npy"))
    ...
    for proto_idx, row in zip(range(0, n_prototypes), img_class_idxs):
        ...
        dest_path = os.path.join(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', str(proto_idx), 'auto_patch_extraction')
        makedir(dest_path)
        print(dest_path)
        for k in range(1, max_k + 1):
            ...
            act_filename = pjoin(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', str(proto_idx), f"nearest-{k}_act.pickle")
            if not os.path.exists(act_filename):
                continue
            act, _ = load(
                pjoin(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', str(proto_idx), f"nearest-{k}_act.pickle"))

            original_img = cv2.imread(
                os.path.join(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', str(proto_idx), f'nearest-{k}_original.png'))
            ...
            extract_highest_activated_patches(original_img,
                                              ...
                                              k)
        print("????????", flush=True)
        print(os.path.join(dest_path, f'exp_*'), flush=True)
...
def interactive_debugging(path):
    ...
    conf_dest_folder = pjoin(os.path.dirname(path),  'tmp_forbidden_conf')
    remember_dest_folder = pjoin(os.path.dirname(path), 'tmp_remember_patch')
    ...
    for img_name in img_paths:
        ...
        model_suffix = path.split('/')[-1].split('.pth.tar')[0]
        act, act_pr = load(
            pjoin(os.path.dirname(path)+f'/{model_suffix}_nearest_train/', str(proto_idx), f"nearest-{img_idx}_act.pickle"))
...
if __name__ == '__main__':
    ...
    if args.strategy == 'single':
        ...
        cfg = OmegaConf.create(
            load(os.path.join(os.path.dirname(args.path_to_folder), 'stat.pickle'))['cfg'])['data']
